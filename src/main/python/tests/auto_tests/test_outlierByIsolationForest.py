...
Outlier score: 0.623"""
            # Get only the last lines to ignore any warnings
            actual_lines = buf.getvalue().strip().split('\n')
            actual_output = '\n'.join([line for line in actual_lines if not line.startswith('Warning:')])
        self.assertEqual(actual_output.strip(), expected)
